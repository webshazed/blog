...
export function buildRecipeSchema(post, siteUrl) {
    ...
    // Add times if found
    if (times.prepTime) schema.prepTime = times.prepTime;
    if (times.cookTime) schema.cookTime = times.cookTime;
    if (times.totalTime) {
        schema.totalTime = times.totalTime;
    } else if (times.prepTime && times.cookTime) {
        // Calculate total from prep + cook
        schema.totalTime = times.prepTime; // Simplified
    }
