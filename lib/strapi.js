...
async function upsertCategory(name) {
    ...
    // Normalize for matching: lowercase, replace spaces and underscores with dashes
    ...
    console.log(`[findCategory] Looking for: name="${name}", normalized="${normalizedName}", slug="${slug}"`);

    try {
        // Fetch ALL categories and do flexible matching (no creation!)
        ...
        if (allCategories.data && allCategories.data.length > 0) {
            ...
            allCategories.data.forEach(c => console.log(`  - "${c.name}" (slug: ${c.slug})`));
        }

        // Fallback to "General" category if provided category not found
        console.log(`[findCategory] Falling back to "General" category`);
        const generalCat = allCategories.data?.find(c =>
            c.name?.toLowerCase() === 'general' || c.slug === 'general'
        );

        if (generalCat) {
            console.log(`[findCategory] Using fallback: id=${generalCat.id}`);
            return generalCat.id;
        }

        // If no categories exist at all, throw error
        throw new Error(`Category "${name}" not found and no fallback available`);
