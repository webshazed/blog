...
export async function getAllArticles(page = 1, pageSize = 10) {
    try {
        const response = await fetchAPI(
            `/articles?populate=*&sort=publishedAt:desc&filters[publishedAt][$notNull]=true&pagination[page]=${page}&pagination[pageSize]=${pageSize}`
        );
        return {
...
export async function getAllAuthors() {
    try {
        const response = await fetchAPI('/authors?populate=*&publicationState=preview');
 ...
 */
async function upsertAuthor(authorInput) {
    // If no input, default to 'Evergreen Team'
    if (!authorInput) authorInput = 'Evergreen Team';

    // If it's a number (ID), use it directly
    const numericId = parseInt(authorInput, 10);
    if (!isNaN(numericId) && numericId > 0) {
        console.log(`[upsertAuthor] Using author ID: ${numericId}`);
        return numericId;
    }

    const name = String(authorInput);
    // Normalize slug: lowercase, replace spaces and underscores with dashes
    ...
    try {
        ...
        // If unique constraint error, try full scan
        ...
        // Fallback: Fetch the first available author instead of hardcoding 1
        console.log('[upsertAuthor] Failed to find/create, fetching any valid author...');
        try {
            const anyAuthor = await fetchAPI('/authors?pagination[limit]=1');
            if (anyAuthor.data && anyAuthor.data.length > 0) {
                const validId = anyAuthor.data[0].id;
                console.log(`[upsertAuthor] Using fallback author ID: ${validId}`);
                return validId;
            }
        } catch (e) {
            console.error('[upsertAuthor] Fallback fetch failed:', e.message);
        }

        console.warn('[upsertAuthor] CRITICAL: No authors found in database due to error. Returning 1 as last resort.');
        return 1;
    }
}
 ...
 */
async function upsertCategory(categoryInput) {
    // If no input, default to category ID 1
    if (!categoryInput) {
        console.log('[findCategory] No category provided, using default ID 1');
        return 1;
    }
    ...
    if (!isNaN(numericId) && numericId > 0) {
        console.log(`[findCategory] Using category ID: ${numericId}`);
        return numericId;
    }

    // Otherwise treat as name/slug and try to find matching category
    const name = String(categoryInput);
    const normalizedName = name.toLowerCase().replace(/[\s_]+/g, ' ').trim();
    const slug = name.toLowerCase()
        .replace(/[\s_]+/g, '-')
        .replace(/[^a-z0-9-]/g, '')
        .replace(/-+/g, '-')
    ...
    console.log(`[findCategory] Looking for: name="${name}", slug="${slug}"`);

    try {
        // Fetch ALL categories and do flexible matching
        ...
        if (allCategories.data && allCategories.data.length > 0) {
            ...
            let match = allCategories.data.find(c => c.slug?.toLowerCase() === slug);

            // Try name variations
            if (!match) {
                match = allCategories.data.find(c => {
                    const catName = c.name?.toLowerCase().replace(/[\s_]+/g, ' ').trim();
            ...
            if (match) {
                console.log(`[findCategory] ✅ Found: id=${match.id}, name="${match.name}"`);
                return match.id;
            }

            // Log available categories for debugging
            console.log(`[findCategory] ⚠️ Category "${name}" not found. Available categories:`);
            allCategories.data.forEach(c => console.log(`  - id=${c.id} "${c.name}" (slug: ${c.slug})`));

            // Return first category as fallback
            console.log(`[findCategory] Using first category as fallback: id=${allCategories.data[0].id}`);
            return allCategories.data[0].id;
        }

        // If no categories exist, use ID 1
        console.log('[findCategory] No categories found, using default ID 1');
        return 1;

    } catch (error) {
        console.error('[findCategory] Error:', error.message);
        console.log('[findCategory] Using default ID 1 due to error');
        return 1;
    }
}
...
function formatArticle(article) {
    ...
    // Handle Strapi v4/v5/v5-alpha response structure variations
    const data = article.attributes || article;
    const authorRaw = data.author?.data?.attributes || data.author?.data || data.author || {};
    const author = authorRaw.attributes || authorRaw;
    const categoryRaw = data.category?.data?.attributes || data.category?.data || data.category || {};
    const category = categoryRaw.attributes || categoryRaw;
    ...
    return {
        ...
        author: author.name || 'Kitchen Algo Team',
        authorSlug: author.slug || '',
        authorBio: author.bio || '',
        authorAvatar: author.avatar || null,
        authorTwitter: author.twitter || null,
        authorLinkedin: author.linkedin || null,
        authorWebsite: author.website || null,
...
function formatAuthor(author) {
    ...
    // Handle Strapi v4/v5 nesting variations
    const data = author.attributes || author;
    const articles = data.articles?.data || data.articles || [];

    // Robustly find avatar URL
    let avatarUrl = null;
    if (data.avatar?.data?.attributes?.url) {
        avatarUrl = data.avatar.data.attributes.url;
    } else if (data.avatar?.url) {
        avatarUrl = data.avatar.url;
    } else if (typeof data.avatar === 'string' && data.avatar.length > 0) {
        // Handle plain string URL (text field case)
        avatarUrl = data.avatar;
    }

    return {
        id: author.id,
        name: data.name || 'Kitchen Algo Author',
        slug: data.slug || '',
