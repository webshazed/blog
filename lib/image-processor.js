...
export async function downloadImage(url, slug, suffix = '') {
    ...
    try {
        ...
        // Goal: significantly reduce size, ideally under 300KB
        // We'll resize if too large and convert to webp with quality adjustment
        ...
        // 3. Upload to R2
        if (R2_ACCOUNT_ID && R2_ACCESS_KEY_ID && R2_SECRET_ACCESS_KEY && !R2_ACCESS_KEY_ID.includes('placeholder')) {
            ...
            // If user provided a custom domain in env, use it. Otherwise construct default if possible (but usually needs mapping).
            // For now assuming R2_PUBLIC_DOMAIN is set correctly or we fall back to a placeholder that user needs to fix.
            const publicUrl = R2_PUBLIC_DOMAIN
                ? `${R2_PUBLIC_DOMAIN}/${filename}`
                : `https://${R2_ACCOUNT_ID}.r2.cloudflarestorage.com/${R2_BUCKET_NAME}/${filename}`; // This default link is usually auth-only, but specific to user config.

            return publicUrl;

        } else {
            console.warn("R2 Credentials missing or placeholder, falling back to local storage.");
