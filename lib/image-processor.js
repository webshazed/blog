...
export async function downloadImage(url, slug, suffix = '') {
    ...
    try {
        ...
        // Invisible SEO #4: Inject Custom Metadata
...
export async function processContentImages(htmlContent, slug) {
    ...
    console.log(`[Image Processor] Found ${uniqueUrls.length} unique images in content for slug: ${slug}`);
    uniqueUrls.forEach((url, i) => console.log(`  [${i}] ${url.substring(0, 80)}...`));
    ...
    for (let i = 0; i < uniqueUrls.length; i++) {
        ...
        if (src.includes(R2_PUBLIC_DOMAIN || 'r2.cloudflarestorage.com')) {
            console.log(`[Image Processor] Skipping already-processed R2 URL: ${src.substring(0, 50)}...`);
            continue;
        }
