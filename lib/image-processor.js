...
export async function processContentImages(htmlContent, slug) {
    ...
    console.log(`\n[Image Processor] Applying ${replacements.size} replacements...`);

    for (const [original, replacement] of replacements) {
        // Check if original URL exists in HTML
        const countBefore = (processedHtml.match(new RegExp(escapeRegExp(original), 'g')) || []).length;
        console.log(`[Replacement] Found ${countBefore} occurrences of: ${original.substring(0, 60)}...`);

        // Replace all occurrences of this URL
        processedHtml = processedHtml.split(original).join(replacement);

        // Verify replacement
        const countAfter = (processedHtml.match(new RegExp(escapeRegExp(original), 'g')) || []).length;
        console.log(`[Replacement] → ${replacement.substring(0, 60)}...`);
        console.log(`[Replacement] Replaced ${countBefore - countAfter} occurrences`);
    }
